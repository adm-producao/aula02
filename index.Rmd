...
grafico_ses <- ggplot(dados_ses_grafico, aes(x = Mes)) +
  ...
  labs(
    ...
    subtitle = "A linha de previsão da SES é ainda mais suave que a das médias móveis.  \n  Com α=0.3, a reação às mudanças na demanda real é lenta e gradual. Isso é bom para filtrar variações aleatórias,  \n mas pode ser ruim para capturar tendências reais rapidamente",
    x = "Mês",
    y = "Demanda (Unidades)"
  ) +
  theme_minimal() +
...
resultados_comparativos <- data.frame(
  Método = c("Média Móvel Simples (n=3)", 
             "Média Móvel Ponderada (0.5,0.3,0.2)", 
             "Suavização Exponencial (α=0.3)"),
  Previsão_Mês_13 = c(161.7, 163.0, 155.9),
  MAPE = c(
    calcular_mape(demanda, calcular_mms(demanda)),
    calcular_mape(demanda, calcular_mmp(demanda)),
    calcular_mape(demanda, calcular_ses(demanda))
  ),
...
[LINK](https://www.places.education/disciplinas/adm_prod_mat/aa02/)
